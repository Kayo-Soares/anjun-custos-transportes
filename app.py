...
# ----------------------------------------------------------------------
@st.cache_data(show_spinner="Lendo e consolidando planilhas...")
def carregar_base(arquivos: list) -> pd.DataFrame:
    partes = []
    for arq in arquivos:
        df = pd.read_excel(arq, sheet_name=SHEET_NAME)
        df = df.rename(columns=COLUMN_MAP)
        # arq pode ser um caminho (str) OU um UploadedFile do Streamlit —
        # cada um guarda o nome do arquivo de um jeito diferente
